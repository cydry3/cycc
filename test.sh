...
try -out 9 "int def() { return 9; } int main(){ a = def(); bar(a);}"
try -out 9 "int def(x) { a = x; bar(a); } int main(){ def(9); }"
try -out 42 "int def(x, y) { a = x + y; bar(a); } int main(){ def(11, 31); }"
try -out 21 "int def(a,b,c,d,e,f) { x = a + b * c; y = d * e - f; buzz(x, y);} int main(){ def(1,2,3,4,5,6); }"
try -out 7 "int def(x, y) { return x + y; } int main(){ a = def(3, 4); bar(a);}"
try -out 5 "int fib(n) { if (n==0) return 0; if (n==1) return 1; return fib(n-2) + fib(n-1); } int main(){ a = fib(5); bar(a);}"
try -out 10946 "int fib(n) { if (n==0) return 0; if (n==1) return 1; return fib(n-2) + fib(n-1); } int main(){ a = fib(21); bar(a);}"
