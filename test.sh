...
try -out OK "int main(){ int foo; foo();}"
try -out 31 "int main(){ int bar; bar(31);}"
try -out 7 "int main(){ int buzz; buzz(3, 4);}"
try -out 21 "int main(){ int fizz; fizz(1, 2, 3, 4, 5, 6);}"
try -out OK "int main(){ int foo; foo(); }"
try -out 31 "int main(){ int bar; bar(31); }"
try -out 7 "int main(){ int buzz; buzz(3, 4); }"
try -out 21 "int main(){ int fizz; fizz(1, 2, 3, 4, 5, 6); }"
try -out OK "int def() { int foo; foo(); } int main(){ int def; def(); }"
try -out 9 "int def() { return 9; } int main(){ int a; int def; int bar; a = def(); bar(a);}"
try -out 9 "int def(int x) { int a; int bar; a = x; bar(a); } int main(){ int def; def(9); }"
try -out 42 "int def(int x, int y) { int a; int bar; a = x + y; bar(a); } int main(){ int def; def(11, 31); }"
try -out 21 "int def(int a, int b, int c, int d, int e, int f) { int x; int y; int buzz; x = a + b * c; y = d * e - f; buzz(x,  y);} int main(){ int def; def(1, 2, 3, 4, 5, 6); }"
try -out 7 "int def(int x, int y) { return x + y; } int main(){ int a; int def; int bar; a = def(3, 4); bar(a);}"
try -out 5 "int fib(int n) { int fib; if (n==0) return 0; if (n==1) return 1; return fib(n-2) + fib(n-1); } int main(){ int a; int fib; int bar; a = fib(5); bar(a);}"
try -out 10946 "int fib(int n) { int fib; if (n==0) return 0; if (n==1) return 1; return fib(n-2) + fib(n-1); } int main(){ int a; int fib; int bar; a = fib(21); bar(a);}"
